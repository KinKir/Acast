...
class Gateway extends Worker
{
    ...
    public function __construct($socket_name, $context_option = [])
    {
        ...
        $this->router = ["\\GatewayWorker\\Gateway", 'routerBind'];
    ...
    public function onWorkerStart()
    {
        ...
        if ($this->lanIp !== '127.0.0.1') {
            Timer::add(self::PERSISTENCE_CONNECTION_PING_INTERVAL, [$this, 'pingBusinessWorker']);
        }

        // 如果 Register 服务器不在本地服务器，则需要保持心跳
        if (strpos($this->registerAddress, '127.0.0.1') !== 0) {
            Timer::add(self::PERSISTENCE_CONNECTION_PING_INTERVAL, [$this, 'pingRegister']);
        }
        ...
        // 设置内部监听的相关回调
        $this->_innerTcpWorker->onMessage = [$this, 'onWorkerMessage'];

        $this->_innerTcpWorker->onConnect = [$this, 'onWorkerConnect'];
        $this->_innerTcpWorker->onClose   = [$this, 'onWorkerClose'];
    ...
    public function registerAddress()
    {
        ...
        $this->_registerConnection->onClose = [$this, 'onRegisterConnectionClose'];
    ...
    {
        foreach ($this->_clientConnections as $client_connection) {
            return call_user_func([$client_connection->protocol, 'encode'], $data, $client_connection);
        }
    }
